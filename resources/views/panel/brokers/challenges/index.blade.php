						...
						@if ($challenges)
							@foreach ($challenges as $challenge)
								<tr>
									...
									@endif
									<td>
										<button type="button" class="btn-sm btn-primary btn mx-1" data-toggle="modal" data-target="#chat{{$challenge->id}}">
								...
								<!-- Modal end -->
								<div class="modal fade" id="chat{{$challenge->id}}">
									{{-- @if ($errors->any())
									...
									<div class="modal-dialog modal-dialog-centered modal-lg">
										<div class="modal-content">
											<div class="modal-header">
												<h5 class="modal-title" id="chat{{$challenge->id}}">پیام جدید:</h5>
												<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
											</div>
											<div class="modal-body">
												<div class="form-group">
													<label for="recipient-name" class="col-form-label">گیرنده:</label>
													<span class="form-control" id="recipient-name">{{$challenge->user->f_name.' '.$challenge->user->l_name}}</span>
												</div>
												<form action="{{route('messagess.store')}}" class="row g-3" method="POST" enctype="multipart/form-data">
													@csrf
													<input type="hidden" name="user_id" value="{{$challenge->user->id}}">
													<div class="form-group">
													...
													<div class="col-12">
														<button type="submit" class="btn btn-primary border-radius-xl">ثبت</button>
														<button type="button" class="btn fw-lighter" data-bs-dismiss="modal">انصراف و برگشت</button>
													</div>
												</form>
											</div>
